...
/**
 * "Smoke" (flow shader) — WebGL1, sin librerías. Triángulo fullscreen.
 * Paleta adaptada a la marca Pinceles (oscuro cálido → ocre → crema) en lugar
 * del azul/cian original del generador, para combinar con el Hero.
 *
 * Colores (low → high), sRGB 0..1:
 *   #14100A  vec3(0.078, 0.063, 0.039)   negro cálido
 *   #8A5A1E  vec3(0.541, 0.353, 0.118)   ámbar profundo
 *   #D9912F  vec3(0.851, 0.569, 0.184)   ocre de marca
 *   #F8F6F1  vec3(0.973, 0.965, 0.945)   crema de la página
 */
...
export function ShaderBackground({
  ...
  /** Overlay opcional (velo) sobre el canvas para legibilidad del contenido. */
  ...
  useEffect(() => {
    ...
    // Feel: warp 0, contraste 69, brillo 50 (centro), saturación 50 (=1.0),
    // hue 0, vignette 0, grain 0, cursor off. Empaquetado según el header.
    gl.uniform4f(uShape, 1.72, 0.6, 0.5, 0.0);
    gl.uniform4f(uSurface, 2.4, 1.22, 0.0, 1.0);
    gl.uniform4f(uFinish, 0.0, 0.0, 0.0, 0.0);
    gl.uniform4f(uTransform, 635.0, 0.0, 0.0, 0.0);
  ...
  return (
    <>
      ...
      {scrim && (
        <div
          aria-hidden
          style={{ position: "absolute", inset: 0, pointerEvents: "none", ...scrim }}
        />
      )}
    </>
  );
